...
int main(int argc, char* argv[]) {
    try {
        ...
        size_t nq, dimq;
        float* xq;
        double time_add_base_set;
        {
            time_add_base_set = elapsed() - t0;
            printf("[%.3f s] Loading queries\n", time_add_base_set);
            xq = fvecs_read(FLAGS_query_file.c_str(), &dimq, &nq);
        }

        size_t ng, kg;
        faiss::idx_t* gt;
        double time_load_query, time_load_gt;

        {
            time_load_query = elapsed() - t0;
            printf("[%.3f s] Loading ground truth for %ld queries\n", time_load_query, nq);

            int* xg = ivecs_read(FLAGS_gt_file.c_str(), &kg, &ng);

            time_load_gt = elapsed() - t0;
